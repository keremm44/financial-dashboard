...
def _bull_fvg_rows() -> list[dict]:
    ...
    rows += [
        _bar(100, 100.0, 100.5, 99.5, 100.0),
        _bar(101, 100.1, 102.0, 100.0, 101.8),
        _bar(102, 101.8, 102.6, 100.8, 102.4),
    ]
    return rows
...
def test_candidate_alignment_adds_only_missing_embedded_contribution() -> None:
    ...
    engine._fvg_formations = [formation]
...
def test_candidate_alignment_does_not_double_count_when_alignment_already_present() -> None:
    ...
    assert engine.fvg_formations[0] == formation
...
def test_open_and_source_gap_bars_cannot_overwrite_last_confirmed_export() -> None:
    ...
    engine.update(_bar(0, 100.0, 101.0, 99.0, 100.5, closed=False))
    assert engine.export == frozen

    engine.update(_bar(0, 100.0, 101.0, 99.0, 100.5, complete=False))
    assert engine.export == frozen
