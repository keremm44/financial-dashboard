...
def _m(**overrides) -> _LifecycleMetrics:
    base = dict(close=105.0, low=104.0, high=106.0, candle_bullish=True, candle_bearish=False, close_location=0.80, body_to_prior_atr=0.8, net_progress_atr=0.8, directional_efficiency=0.8, higher_close_share=0.75, lower_close_share=0.0, buy_continuation_candidate=True, sell_continuation_candidate=False, buy_continuation_confirmed=True, sell_continuation_confirmed=False, lower_rejection=False, upper_rejection=False, bullish_engulfing=False, bearish_engulfing=False)
...
def _fvg(*, bullish=True, state=FvgState.ACTIVE, formation_index=100, quality=70.0) -> FvgLifecycleRecord:
    return FvgLifecycleRecord(direction=FvgDirection.BULLISH if bullish else FvgDirection.BEARISH, state=state, lower_boundary=100.0, upper_boundary=110.0, gap_size=10.0, gap_atr=1.0, formation_atr=10.0, invalidation_buffer=0.5, formation_index=formation_index, formation_time=pd.Timestamp("2026-01-01", tz=TZ), quality=quality, evidence_count=8)


def _engulf(*, bullish=True, state=EngulfingState.ACTIVE, formation_index=100, quality=75.0) -> EngulfingLifecycleRecord:
    return EngulfingLifecycleRecord(direction=EngulfingDirection.BULLISH if bullish else EngulfingDirection.BEARISH, state=state, lower_boundary=100.0, upper_boundary=110.0, body_size=10.0, body_atr=1.0, formation_high=112.0, formation_low=98.0, formation_index=formation_index, formation_time=pd.Timestamp("2026-01-01", tz=TZ), quality=quality)


def test_bullish_fvg_partial_and_deep_fill_states_follow_source_thresholds() -> None:
    engine = FvgEngulfingEngine(); engine._bull_fvg = _fvg()
    ...
    assert engine.active_bullish_fvg is not None and engine.active_bullish_fvg.state is FvgState.PARTIAL_FILL
    ...
    assert engine.active_bullish_fvg is not None and engine.active_bullish_fvg.state is FvgState.DEEP_TEST
...
def test_fvg_close_invalidation_uses_frozen_formation_buffer_and_source_priority() -> None:
    engine = FvgEngulfingEngine(FvgEngulfingConfig(sensitivity=SensitivityProfile.SENSITIVE)); engine._bull_fvg = _fvg()
...
def test_candidate_promotion_can_preserve_same_bar_first_test() -> None:
    engine = FvgEngulfingEngine(); engine._bull_fvg = _fvg(state=FvgState.CANDIDATE)
    engine._update_fvg_record(True, 101, _m(close=111.0, low=108.0, high=112.0))
    rec = engine.active_bullish_fvg
    assert rec is not None and rec.state is FvgState.FIRST_TEST and rec.tested and rec.first_test_index == 101
...
def test_bullish_fvg_reaction_is_terminal_after_zone_hold_and_away_move() -> None:
    engine = FvgEngulfingEngine(); rec = _fvg(); rec.tested = True; rec.first_test_index = 100; rec.maximum_fill_ratio = 0.30; engine._bull_fvg = rec
    engine._update_fvg_record(True, 102, _m(close=112.0, low=111.0, high=113.0))
    assert engine.active_bullish_fvg is None
    assert engine.completed_fvg[-1].state is FvgState.REACTION and engine.completed_fvg[-1].reaction_confirmed
...
def test_fvg_takeover_marks_existing_record_superseded_and_freezes_new_quality() -> None:
    ...
    engine = FvgEngulfingEngine(); engine._bull_fvg = _fvg(quality=60.0)
    formation = FvgFormation(FvgDirection.BULLISH, FvgState.ACTIVE, 110, pd.Timestamp("2026-01-02", tz=TZ), 105.0, 115.0, 10.0, 1.0, 10.0, 70.0, 10.0, 9)
    engine._accept_fvg_formation(formation, 110, _m(close=116.0))
    assert engine.completed_fvg[-1].state is FvgState.SUPERSEDED
    assert engine.active_bullish_fvg is not None and engine.active_bullish_fvg.quality == 70.0
...
def test_engulfing_retrace_then_weakened_then_grace_expiry() -> None:
    engine = FvgEngulfingEngine(); engine._bull_engulf = _engulf()
    ...
    assert engine.active_bullish_engulfing is not None and engine.active_bullish_engulfing.state is EngulfingState.PARTIAL_RETRACE
    ...
    assert engine.active_bullish_engulfing is not None and engine.active_bullish_engulfing.state is EngulfingState.WEAKENED
    ...
    assert engine.active_bullish_engulfing is None and engine.completed_engulfing[-1].state is EngulfingState.EXPIRED
...
def test_export_contract_uses_remaining_fvg_zone_original_engulf_zone_and_signed_bear_states() -> None:
    engine = FvgEngulfingEngine(); bull = _fvg(); bull.maximum_fill_ratio = .40; bull.state = FvgState.PARTIAL_FILL; bear = _fvg(bullish=False); bear.maximum_fill_ratio = .30; bear.state = FvgState.PARTIAL_FILL; be = _engulf(bullish=False); be.maximum_retrace_ratio = .35; be.state = EngulfingState.PARTIAL_RETRACE
    engine._bull_fvg, engine._bear_fvg, engine._bear_engulf = bull, bear, be
    engine._bull_fvg_event = (120, FvgState.REACTION); engine._bear_fvg_event = (120, FvgState.FULL_FILL); engine._bear_engulf_event = (120, EngulfingState.INVALID)
    ex = engine._build_export(120)
    assert ex.bull_fvg.state == int(FvgState.PARTIAL_FILL) and ex.bull_fvg.top == pytest.approx(106.0) and ex.bull_fvg.bottom == 100.0 and ex.bull_fvg.event == int(FvgState.REACTION)
    assert ex.bear_fvg.state == -int(FvgState.PARTIAL_FILL) and ex.bear_fvg.top == 110.0 and ex.bear_fvg.bottom == pytest.approx(103.0) and ex.bear_fvg.event == -int(FvgState.FULL_FILL)
    assert ex.bear_engulf.state == -int(EngulfingState.PARTIAL_RETRACE) and ex.bear_engulf.top == 110.0 and ex.bear_engulf.bottom == 100.0
    assert ex.bear_engulf.retrace == pytest.approx(.35) and ex.bear_engulf.event == -int(EngulfingState.INVALID)


def test_directional_terminal_events_do_not_overwrite_each_other_same_bar() -> None:
    engine = FvgEngulfingEngine(); engine._bull_fvg_event = (50, FvgState.REACTION); engine._bear_fvg_event = (50, FvgState.FULL_FILL)
    ex = engine._build_export(50)
    assert ex.bull_fvg.event == int(FvgState.REACTION) and ex.bear_fvg.event == -int(FvgState.FULL_FILL)


def test_event_ports_disappear_after_most_recent_closed_snapshot_bar() -> None:
    engine = FvgEngulfingEngine(); engine._bull_fvg_event = (50, FvgState.REACTION)
