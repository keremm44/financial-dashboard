...
def test_freeze_without_real_cross_regime_evidence_fails_closed():
    ...
    assert "ST_FREEZE_MULTIPLE_MARKET_REGIMES_REQUIRED" in assessment.blockers
    assert "ST_FREEZE_CROSS_REGIME_REVIEW_REQUIRED" in assessment.blockers
...
def test_duplicate_regime_identity_or_period_cannot_fake_cross_regime_coverage():
    ...
    assert "ST_FREEZE_REGIME_IDS_MUST_BE_DISTINCT" in by_id.blockers
    assert "ST_FREEZE_REGIME_PERIODS_MUST_BE_DISTINCT" in by_period.blockers
...
def test_cross_regime_review_must_accept_both_early_and_late_behavior_axes():
    ...
    assert assessment.status is STImplementationFreezeStatus.VALIDATION_REQUIRED
    assert "ST_FREEZE_CROSS_REGIME_BEHAVIOR_NOT_ACCEPTED" in assessment.blockers
