...
## Market Structure separation

Primary structure and structural events are separate facts.

- `MARKET_STRUCTURE` is the external/primary structural state plus source-approved score and protected/weak levels.
- If the optional `EngineResult` is absent, TEL recovers primary direction from the signed public `external_state` export. It does not promote `internal_state` when external state is unavailable.
- Confirmed/latest public structural events are exported as structured external/internal event ports rather than recovered from `EngineResult.events` strings.
- BOS, CHoCH, SWEEP, FALSE_BREAK and TRANSITION_FAIL can therefore reach TEL as `TRIGGER` evidence with event bar, direction, level, identity, quality and broken/origin provenance.
- Primary structure and its event ports remain in the same `MARKET_STRUCTURE_CORE` independence group; they are not flat independent votes.

The Decision Engine must not parse Market Structure event strings.
...
## Support/Resistance separation

Range geometry and confirmed range breaks are separate facts.

- Range boundaries, support/resistance zones, role-reversal zones and break boundary remain `LOCATION` evidence.
- `RANGE_BREAK_CONFIRMED` becomes a separate `SUPPORT_RESISTANCE_BREAK` trigger only when the public contract supplies a non-zero break direction and `break_confirmed_index`.
- The confirmed break event may use `break_confirmed_index` as its causal event bar.
- The break boundary itself does not inherit that bar as a formation/source bar; its origin remains unknown unless the source contract exposes it.

## Source bar discipline

`source_bar` means causal origin/event bar, never merely a known/confirmation index for a different fact.

Therefore:

- Order Block source bars remain source bars because the public contract supplies them.
- Market Structure structured event ports use their public `event_bar` as the event source bar.
...
- Support/Resistance confirmed break evidence may use `break_confirmed_index`; the boundary level itself remains unanchored.
...
The public bundle gate rejects:
...
- member evidence/levels whose supplied `known_bar` does not equal the containing packet snapshot;
- member evidence/levels whose supplied timestamp does not equal the containing packet timestamp.
...
When an explicit `as_of_timestamp` is supplied, an incomparable packet timestamp (for example naive versus timezone-aware) fails closed instead of bypassing the future-data guard.
...
MTF Story itself only consumes Market Structure and Pattern normalized timeframe inputs. TEL may conservatively represent lineage at the engine-family level, but the derived Story remains non-independent regardless; root evidence is never removed by that linkage.
...
Where a source provides stable identity metadata (for example Liquidity identity, Market Structure event identity or Order Block source bar), the Decision Engine may retain it as supporting provenance for that setup.
...
- structured Market Structure and Liquidity events;
- confirmed Support/Resistance break triggers;
- provenance/lineage;
- independence groups;
