...
def resample_daily_to_weekly(
    ...
    *,
    ...
    """Aggregate daily bars into deterministic Friday-close weekly bars.

    Holiday weeks may contain fewer than five trading sessions, so completeness is
    based on source-bar integrity plus whether the week itself has closed, not on a
    fixed source count.
    """
    if daily.empty:
    ...
    work["week_start"] = day - pd.to_timedelta(localized.dt.weekday, unit="D")
...
class DailyContextPipeline:
    """Incremental daily cache plus locally derived weekly context."""
