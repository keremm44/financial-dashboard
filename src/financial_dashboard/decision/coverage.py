...
def assess_coverage(
    ...
    *,
    ...
    Fractions are descriptive breadth metrics only. They are not confidence scores,
    votes or probabilities, and no family weight is embedded here.
    """
    ...
    for family in expected:
        quality = family_quality.get(family, ContextDataQuality.UNAVAILABLE)
    ...
    critical_missing = tuple(
        ...
        if family_quality.get(family, ContextDataQuality.UNAVAILABLE) is not ContextDataQuality.VALID
    )
    total = len(expected)
